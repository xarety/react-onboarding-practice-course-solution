...
import { toJS, isArrayLike, runInAction, transaction, computed, comparer } from 'mobx';
...
export class CheckboxFieldState extends FieldState<boolean> {
    ...
    DEPRECATED_onChangeHandler = (
        _0: React.SyntheticEvent<HTMLInputElement>,
...
export class InputFieldState<T> extends FieldState<T> {
    ...
    onChangeNativeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
...
export function setFormStateValues<T extends ValidatableMapOrArray>(
    ...
    if (isArrayLike(formState.$) && (data as any[]).length !== formState.$.length) {
        throw new Error(
